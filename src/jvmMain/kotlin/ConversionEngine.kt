...
fun generateKeyValueMap(groupKey: CSVGroupKey, eintrage: List<CSVEntry>): MutableMap<String, Any> {
    ...
    keyValueMap["od_schutzgasnumber"] = randomEintrag[CSVHeader.Shield_Gas_Number.ordinal]
    ...
    keyValueMap["id_schutzgasnumber"] = randomEintrag[CSVHeader.ID_Gas_Number.ordinal]
...
fun convertToPDF(csvFile: File?, outputFolder: Path) {
    ...
    for ((groupKey, eintrage) in csvGrouped) {
        ...
        val pdfName = if (wpsMap[groupKey.second] != null) {
            "Weldlog_${projectName}_${auftragsnummer}_${interneNummer}_${weldProgram}.pdf"
        } else {
            "WPS NICHT GEFUNDEN - Weldlog_${projectName}_${auftragsnummer}_${interneNummer}_${weldProgram}.pdf"
        }
